...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("nervoscan-auth-change", authHandler);
    
    return () => {
      window.removeEventListener("nervoscan-locale-change", handler);
      window.removeEventListener("nervoscan-auth-change", authHandler);
    };
  }, []);
    ...
    <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="text-center max-w-2xl mx-auto">
        ...
        <div className="mt-12 flex flex-col items-center gap-3">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700/50 text-xs text-slate-400">
            <svg className="w-3.5 h-3.5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
            </svg>
            {t("privacy.note", locale)}
          </div>
          <Link href="/terms" className="text-xs text-slate-500 hover:text-indigo-400 transition-colors underline underline-offset-2">
